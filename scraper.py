...
def is_valid_report(icao, raw_report):
    """
    放寬校驗邏輯：只要包含機場代碼與時間格式即視為有效，避免因花蓮報文格式特殊被剔除。
    """
    if not raw_report: return False
    # 只要有 機場代碼 + 時間 + 風速(KT) 就判定為有效
        ...
        try:
            ...
            for icao, item in text_result.items():
                if icao not in result or int(item["obsTime"]) >= int(result[icao]["obsTime"]):
                    result[icao] = item
            if "RCYU" in result: log("✅ AOAWS 成功抓取花蓮 (RCYU)")
            return result
        except Exception as e:
...
def fetch_from_aviationweather():
    """
    備援 API：將抓取範圍擴大至 12 小時，確保花蓮關場後仍能顯示最後一筆報文。
    """
    ids = ",".join(AIRPORTS)
    ...
    try:
        ...
        for item in data:
            ...
            if icao in AIRPORTS and is_valid_report(icao, raw_report):
                ...
                new_item = {"icaoId": icao, "obsTime": obs_time, "rawOb": raw_report, "visib": visibility, "status": "updated"}
                if icao not in result or obs_time > int(result[icao]["obsTime"]):

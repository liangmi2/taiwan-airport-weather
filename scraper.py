import os
import re
import json
import time
import html
import calendar
import requests
from datetime import datetime, timezone, timedelta


# 首頁要顯示的 8 個機場
AIRPORTS = [
    "RCTP",  # 桃園國際機場
    "RCSS",  # 台北松山機場
    "RCKH",  # 高雄小港機場
    "RCMQ",  # 台中清泉崗機場
    "RCBS",  # 金門尚義機場
    "RCNN",  # 台南機場
    "RCYU",  # 花蓮機場
    "RCQC"   # 馬公機場
]

AOAWS_API_URL = "https://aoaws.anws.gov.tw/Report/get_metar_speci"
AOAWS_REFERER = "https://aoaws.anws.gov.tw/Report#gsc.tab=0"


def log(*args):
    print(*args, flush=True)


def parse_metar_obs_time(raw_report):
    try:
        match = re.search(r"\b(\d{2})(\d{2})(\d{2})Z\b", raw_report)

        if not match:
            return 0

        day, hour, minute = map(int, match.groups())

        now = datetime.now(timezone.utc)
        year = now.year
        month = now.month

        last_day = calendar.monthrange(year, month)[1]

        if day > last_day:
            return 0

        dt = datetime(
            year,
            month,
            day,
            hour,
            minute,
            tzinfo=timezone.utc
        )

        # 處理跨月
        if dt - now > timedelta(days=1):
            month -= 1

            if month == 0:
                month = 12
                year -= 1

            last_day = calendar.monthrange(year, month)[1]

            if day > last_day:
                return 0

            dt = datetime(
                year,
                month,
                day,
                hour,
                minute,
                tzinfo=timezone.utc
            )

        return int(dt.timestamp())

    except Exception:
        return 0


def parse_visibility_from_raw(raw_report):
    """
    台灣 METAR 常以 4 位數公尺表示能見度，例如：
    9000 = 9000 公尺
    9999 = 10 公里以上
    """

    match = re.search(r"\s(\d{4})\s", raw_report)

    if not match:
        return ""

    meters = int(match.group(1))

    if meters == 9999:
        return "6.2"

    return str(round(meters / 1609.34, 2))


def normalize_raw_report(raw):
    if not raw:
        return ""

    raw = html.unescape(str(raw))

    raw = (
        raw
        .replace("\\n", " ")
        .replace("\\/", "/")
        .replace('\\"', '"')
        .replace("\u00a0", " ")
        .replace("=", "")
    )

    raw = re.sub(r"\s+", " ", raw).strip()

    if raw.startswith("METAR ") or raw.startswith("SPECI "):
        return raw

    if re.match(r"^[A-Z]{4}\s+\d{6}Z", raw):
        return "METAR " + raw

    return raw


def is_valid_report(icao, raw_report):
    """
    基本有效性檢查：
    1. 有 METAR / SPECI
    2. 有正確 ICAO
    3. 有觀測時間
    4. 有 KT 風速資訊
    """

    if not raw_report:
        return False

    pattern = rf"\b(?:METAR|SPECI)\s+{icao}\s+\d{{6}}Z\b"

    if not re.search(pattern, raw_report):
        return False

    if "KT" not in raw_report:
        return False

    return True


def make_weather_item(
    icao,
    raw_report,
    source="AOAWS",
    fetched_at=None
):
    raw_report = normalize_raw_report(raw_report)

    if fetched_at is None:
        fetched_at = int(time.time())

    return {
        "icaoId": icao,
        "obsTime": parse_metar_obs_time(raw_report),
        "rawOb": raw_report,
        "visib": parse_visibility_from_raw(raw_report),
        "status": "updated",
        "source": source,
        "fetchedAt": fetched_at
    }


def load_old_weather():
    if not os.path.exists("local_weather.json"):
        return {}

    try:
        with open(
            "local_weather.json",
            "r",
            encoding="utf-8"
        ) as f:
            old_data = json.load(f)

        if not isinstance(old_data, list):
            return {}

        return {
            item.get("icaoId"): item
            for item in old_data
            if item.get("icaoId")
        }

    except Exception as e:
        log("讀取舊資料失敗：", e)
        return {}


def collect_report_items(obj):
    items = []

    if isinstance(obj, list):
        for x in obj:
            items.extend(collect_report_items(x))

    elif isinstance(obj, dict):
        if "content" in obj and "station" in obj:
            items.append(obj)

        for value in obj.values():
            if isinstance(value, (list, dict)):
                items.extend(collect_report_items(value))

    return items


def extract_reports_from_response_text(
    response_text,
    stations=None
):
    result = {}

    if not response_text:
        return result

    if stations is None:
        stations = AIRPORTS

    text = html.unescape(response_text)

    text = (
        text
        .replace("\\/", "/")
        .replace("\\n", " ")
        .replace('\\"', '"')
        .replace("\u00a0", " ")
    )

    text = re.sub(r"\s+", " ", text)

    fetched_at = int(time.time())

    for icao in stations:

        # 第一種：有 = 結尾
        pattern = (
            rf"((?:METAR|SPECI)\s+{icao}\s+"
            rf"\d{{6}}Z\s+.*?=)"
        )

        matches = list(re.finditer(pattern, text))

        for match in matches:
            raw_report = normalize_raw_report(
                match.group(1)
            )

            if not is_valid_report(
                icao,
                raw_report
            ):
                continue

            item = make_weather_item(
                icao,
                raw_report,
                source="AOAWS",
                fetched_at=fetched_at
            )

            if (
                icao not in result
                or int(item.get("obsTime", 0))
                >= int(
                    result[icao].get(
                        "obsTime",
                        0
                    )
                )
            ):
                result[icao] = item

    return result


def build_aoaws_payload(stations):
    """
    AOAWS API 使用同一個 search_airports[] 欄位
    傳送多個機場代碼。
    """

    payload = []

    for icao in stations:
        payload.append(
            ("search_airports[]", icao)
        )

    payload.extend([
        ("time_item", "recent_time"),
        ("time_select", "1")
    ])

    return payload


def fetch_from_aoaws_api(
    stations=None,
    label="批次"
):
    """
    從台灣航空氣象服務網 AOAWS 抓取 METAR。

    stations 可指定機場，例如：
    ["RCBS"]

    這也是金門單站重試使用的函式。
    """

    if stations is None:
        stations = AIRPORTS

    stations = list(stations)

    log(
        f"🌐 正在抓取 AOAWS 官方 API "
        f"({label})："
        f"{', '.join(stations)}"
    )

    result = {}

    headers = {
        "User-Agent": (
            "Mozilla/5.0 "
            "(Windows NT 10.0; Win64; x64) "
            "AppleWebKit/537.36 "
            "(KHTML, like Gecko) "
            "Chrome/125.0 Safari/537.36"
        ),
        "Content-Type": (
            "application/x-www-form-urlencoded; "
            "charset=UTF-8"
        ),
        "Origin": "https://aoaws.anws.gov.tw",
        "Referer": AOAWS_REFERER,
        "X-Requested-With": "XMLHttpRequest",
        "Accept": (
            "application/json, text/javascript, "
            "*/*; q=0.01"
        )
    }

    payload = build_aoaws_payload(stations)

    for attempt in range(1, 4):
        try:
            response = requests.post(
                AOAWS_API_URL,
                headers=headers,
                data=payload,
                timeout=30
            )

            response.raise_for_status()

            fetched_at = int(time.time())

            # -------------------------
            # 方法 1：優先解析 JSON
            # -------------------------
            try:
                payload_json = response.json()

                report_items = collect_report_items(
                    payload_json
                )

                for item in report_items:

                    icao = str(
                        item.get("station") or ""
                    ).upper().strip()

                    raw_report = normalize_raw_report(
                        item.get("content", "")
                    )

                    if icao not in stations:
                        continue

                    if not is_valid_report(
                        icao,
                        raw_report
                    ):
                        continue

                    weather_item = make_weather_item(
                        icao,
                        raw_report,
                        source="AOAWS",
                        fetched_at=fetched_at
                    )

                    if (
                        icao not in result
                        or int(
                            weather_item[
                                "obsTime"
                            ]
                        )
                        >= int(
                            result[icao].get(
                                "obsTime",
                                0
                            )
                        )
                    ):
                        result[icao] = (
                            weather_item
                        )

            except Exception as e:
                log(
                    "AOAWS JSON 解析略過：",
                    e
                )

            # -------------------------
            # 方法 2：文字正則備援
            # -------------------------
            text_result = (
                extract_reports_from_response_text(
                    response.text,
                    stations
                )
            )

            for icao, item in (
                text_result.items()
            ):
                if (
                    icao not in result
                    or int(
                        item.get(
                            "obsTime",
                            0
                        )
                    )
                    >= int(
                        result[icao].get(
                            "obsTime",
                            0
                        )
                    )
                ):
                    result[icao] = item

            log(
                f"✅ AOAWS {label}完成，"
                f"取得 {len(result)} 個機場"
            )

            return result

        except Exception as e:
            log(
                f"⚠️ AOAWS {label}"
                f"第 {attempt} 次失敗：",
                e
            )

            if attempt < 3:
                time.sleep(5)

    return result


def fetch_rcbs_direct():
    """
    金門尚義機場 RCBS 專用重試。

    原因：
    AOAWS 批次查詢偶爾可能沒有把 RCBS
    放進回傳結果，但官方網站實際已有最新報文。

    因此批次抓取完後，RCBS 若不存在，
    再單獨向 AOAWS 查一次。
    """

    log(
        "🔁 批次資料沒有 RCBS，"
        "啟動金門尚義機場單站重試..."
    )

    result = fetch_from_aoaws_api(
        stations=["RCBS"],
        label="RCBS 單站重試"
    )

    if "RCBS" in result:
        result["RCBS"]["source"] = (
            "AOAWS-RCBS"
        )

        log(
            "✅ RCBS 單站重試成功：",
            result["RCBS"].get(
                "rawOb",
                ""
            )
        )

    else:
        log(
            "⚠️ RCBS 單站重試仍未取得資料"
        )

    return result


def fetch_from_aviationweather(
    stations=None
):
    """
    美國 AviationWeather 備援。

    只有 AOAWS 抓不到的機場
    才使用此來源。

    時間範圍 12 小時，
    讓部分夜間停止發布報文的機場
    還能保留最近資料。
    """

    if stations is None:
        stations = AIRPORTS

    stations = list(stations)

    if not stations:
        return {}

    ids = ",".join(stations)

    url = (
        "https://aviationweather.gov/"
        "api/data/metar"
        f"?ids={ids}"
        "&format=json"
        "&taf=false"
        "&hours=12"
    )

    log(
        "🌐 正在抓取 AviationWeather "
        "備援：",
        ids
    )

    result = {}

    try:
        response = requests.get(
            url,
            timeout=30,
            headers={
                "User-Agent":
                "taiwan-airport-weather/1.0"
            }
        )

        response.raise_for_status()

        data = response.json()

        fetched_at = int(time.time())

        for item in data:

            icao = str(
                item.get("icaoId") or ""
            ).upper().strip()

            if icao not in stations:
                continue

            raw_report = normalize_raw_report(
                item.get("rawOb", "")
            )

            if not is_valid_report(
                icao,
                raw_report
            ):
                continue

            obs_time = parse_metar_obs_time(
                raw_report
            )

            visib = str(
                item.get("visib", "")
            ).replace("+", "")

            if not visib:
                visib = (
                    parse_visibility_from_raw(
                        raw_report
                    )
                )

            new_item = {
                "icaoId": icao,
                "obsTime": obs_time,
                "rawOb": raw_report,
                "visib": visib,
                "status": "updated",
                "source":
                    "AviationWeather",
                "fetchedAt":
                    fetched_at
            }

            if (
                icao not in result
                or obs_time
                > int(
                    result[icao].get(
                        "obsTime",
                        0
                    )
                )
            ):
                result[icao] = new_item

        log(
            "✅ AviationWeather 備援完成，"
            f"取得 {len(result)} 個機場"
        )

    except Exception as e:
        log(
            "⚠️ AviationWeather "
            "備援抓取失敗：",
            e
        )

    return result


def fetch_aoaws_metar():

    run_time = int(time.time())

    old_weather = load_old_weather()

    # =================================
    # 1. AOAWS 全機場批次抓取
    # =================================
    aoaws_weather = (
        fetch_from_aoaws_api(
            AIRPORTS,
            label="全機場批次"
        )
    )

    # =================================
    # 2. 金門 RCBS 專用單站補抓
    # =================================
    if "RCBS" not in aoaws_weather:

        rcbs_result = (
            fetch_rcbs_direct()
        )

        if "RCBS" in rcbs_result:
            aoaws_weather["RCBS"] = (
                rcbs_result["RCBS"]
            )

    # =================================
    # 3. 找出仍缺少的機場
    # =================================
    missing = [
        icao
        for icao in AIRPORTS
        if icao not in aoaws_weather
    ]

    if missing:
        log(
            "⚠️ AOAWS 尚缺：",
            ", ".join(missing)
        )

    # =================================
    # 4. AviationWeather 備援
    # =================================
    aviation_weather = {}

    if missing:
        aviation_weather = (
            fetch_from_aviationweather(
                missing
            )
        )

    # =================================
    # 5. 合併資料
    # AOAWS 永遠優先
    # =================================
    new_weather = dict(
        aoaws_weather
    )

    for icao, item in (
        aviation_weather.items()
    ):
        if icao not in new_weather:
            new_weather[icao] = item

    # =================================
    # 6. 建立輸出資料
    # =================================
    weather_data = []

    for icao in AIRPORTS:

        if icao in new_weather:

            item = dict(
                new_weather[icao]
            )

            item["checkedAt"] = run_time

            weather_data.append(
                item
            )

        elif icao in old_weather:

            # 本輪完全抓不到時，
            # 保留上一筆資料，
            # 避免網頁突然整張空白
            item = dict(
                old_weather[icao]
            )

            item["status"] = (
                "old_data_kept"
            )

            item["checkedAt"] = (
                run_time
            )

            weather_data.append(
                item
            )

            log(
                f"🟡 {icao} 本輪無新資料，"
                "保留上一筆"
            )

        else:

            weather_data.append({
                "icaoId": icao,
                "obsTime": 0,
                "rawOb": "",
                "visib": "",
                "status": "暫無報文",
                "source": "",
                "fetchedAt": 0,
                "checkedAt": run_time
            })

    # =================================
    # 7. 寫入 JSON
    # =================================
    with open(
        "local_weather.json",
        "w",
        encoding="utf-8"
    ) as f:

        json.dump(
            weather_data,
            f,
            ensure_ascii=False,
            indent=4
        )

    # =================================
    # 8. 額外顯示 RCBS 狀況
    # =================================
    rcbs = next(
        (
            x
            for x in weather_data
            if x.get("icaoId")
            == "RCBS"
        ),
        None
    )

    if rcbs:
        log(
            "🇹🇼 金門 RCBS："
        )
        log(
            "來源：",
            rcbs.get(
                "source",
                "--"
            )
        )
        log(
            "METAR：",
            rcbs.get(
                "rawOb",
                "--"
            )
        )

    log(
        "🎉 氣象資料更新完成，"
        "local_weather.json 已寫入。"
    )


if __name__ == "__main__":
    fetch_aoaws_metar()
